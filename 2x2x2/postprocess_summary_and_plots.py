...
Fixed-size branch version for 2x2x2.

This script does both:
1) Read Results_2x2x2 or Results_4x4x4 CSV files
2) Create summary_metrics.csv
3) Create validation plots in Results_2x2x2/figures or Results_4x4x4/figures

Run with normal Python, not Abaqus Python:
    python postprocess_summary_and_plots.py

Run from this branch folder:
    python postprocess_summary_and_plots.py
"""
...
DEFAULT_SPECIMEN_HEIGHT_MM = 40
DEFAULT_SPECIMEN_AREA_MM2 = 1600
DEFAULT_SPECIMEN_VOLUME_MM3 = DEFAULT_SPECIMEN_HEIGHT_MM * DEFAULT_SPECIMEN_AREA_MM2
...
def configure_run_size(run_size):
    global RUN_SIZE, RESULT_DIR_NAME
    RUN_SIZE = infer_run_size_from_text(run_size, RUN_SIZE)
    RESULT_DIR_NAME = "Results"


def get_specimen_geometry(case_name):
    run_size = RUN_SIZE
    height = 40
    area = 1600
    volume = height * area
    return {"run_size": run_size, "height": height, "area": area, "volume": volume}
...
def calculate_case_metrics(case_name, result_dir, design_info):
    ...
    geom = get_specimen_geometry(case_name)
    ...
    initial_stiffness = early_fit_slope(
        fd_rows,
        x_max=INITIAL_FIT_STRAIN_MAX * geom["height"],
    ...
    if SOLID_DENSITY_G_CM3 is not None and rel_density is not None:
        ...
        mass_g = solid_density_g_mm3 * rel_density * geom["volume"]
        if mass_g > 0:
    ...
    return {
        ...
        "specimen_size": geom["run_size"],
        "specimen_height_mm": geom["height"],
        "specimen_area_mm2": geom["area"],
...
def write_summary(out_path, records):
    header = [
        ...
        "specimen_size", "specimen_height_mm", "specimen_area_mm2",
...
def main():
    ...
    print("RESULT_DIR_NAME =", RESULT_DIR_NAME)
    print("============================================================")
